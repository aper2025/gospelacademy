...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Link teacher to student (teachers only)
  app.post('/api/teacher/link-student', requireAuth, async (req: any, res) => {
    try {
      const userId = req.currentUser.id;
      const user = await storage.getUser(userId);
      
      if (user?.role !== 'teacher') {
        return res.status(403).json({ message: "Only teachers can link students" });
      }
  ...
  app.get('/api/teacher/my-students', requireAuth, async (req: any, res) => {
    try {
      const userId = req.currentUser.id;
      const user = await storage.getUser(userId);
      ...
      const students = await db
        .select({
          ...
          email: users.email,
          linkedAt: teacherStudentLinks.linkedAt,
        })
        .from(teacherStudentLinks)
        .innerJoin(users, eq(teacherStudentLinks.studentId, users.id))
        .where(and(
          eq(teacherStudentLinks.teacherId, userId),
          eq(teacherStudentLinks.isActive, true)
        ));
  ...
  app.get('/api/messages/received', requireAuth, async (req: any, res) => {
    try {
      const userId = req.currentUser.id;
  ...
  app.get('/api/messages/sent', requireAuth, async (req: any, res) => {
    try {
      const userId = req.currentUser.id;
  ...
  // Find students by email (for teachers to link)
  app.get('/api/teacher/find-students', requireAuth, async (req: any, res) => {
    try {
      const userId = req.currentUser.id;
      const user = await storage.getUser(userId);
